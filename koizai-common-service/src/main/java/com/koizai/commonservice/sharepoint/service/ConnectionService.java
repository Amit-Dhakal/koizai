...
public class ConnectionService {
    ...
    private static String generateBearerToken(String bearerRealm, String resourceId, CloseableHttpClient httpClient) {
        ...
        String clientId = String.format("%s@%s", ConstantUtil.clientId, bearerRealm);
        String resource = String.format("%s/%s@%s", resourceId, ConstantUtil.domain + ".sharepoint.com", bearerRealm);

        List<NameValuePair> params = Lists.newArrayList(
                new BasicNameValuePair("grant_type", "client_credentials"),
                new BasicNameValuePair("client_id", clientId),
                new BasicNameValuePair("client_secret", ConstantUtil.clientSecret),
    ...
    private static List<String> getBearerRealmAndResourceId(CloseableHttpClient httpClient) {
        String url = String.format("https://%s/_layouts/15/sharepoint.aspx", ConstantUtil.domain + ".sharepoint.com");
